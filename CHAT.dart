                              ...
                              if(snapshot.hasData && !snapshot.hasError){
                                ...
                                if(userHandler == UserType.NORMAL){
                                  messages = snapshot.data!.docs
                                      .where((element) => element['subscriber'] == totalContacts[i]).toList();

                                  nameHolder = 'subscriber_name';
                                }else{
                                  messages = snapshot.data!.docs
                                      .where((element) => element['publisher'] == totalContacts[i]).toList();

                                  nameHolder = "publisher_name";
                                }
                                messages.sort((a,b){
                                  return b.get("timestamp").toString().compareTo(a['timestamp'].toString());
                                });
                                      ...
                                      InkWell(
                                        ...
                                        child: Container(
                                          ...
                                          child: ListTile(
                                            ...
                                            trailing: Text("${DateTime.fromMillisecondsSinceEpoch(messages.last['timestamp']).toLocal()}",style: const TextStyle(fontWeight: FontWeight.normal,
                                                fontStyle: FontStyle.italic, fontSize: 10,fontFamily: 'ultra' ,color: Colors.black)),
                                            subtitle: Text("${messages.last['content']}",style: const TextStyle(fontWeight: FontWeight.normal,
